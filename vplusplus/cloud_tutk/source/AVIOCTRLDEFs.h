...
{
    unsigned int num;
	unsigned int channel[8]; // Camera Index
...
typedef struct
{
	unsigned int channel; // Camera Index, -1 means all
	char userdata[64];
	unsigned char reserved[4];
...
{
	unsigned int channel; 		// Camera Index
	int     tmbegin; 		// Search event from ...
	int     tmend; 	  		// ... to (search event)
	unsigned int type;  		// event type, user defined
}SMsgAVIoctrlListYGEventReq;


/*
IOTYPE_USER_IPCAM_LIST_YGEVENT_RESP
*/
typedef struct
{
	unsigned int  channel;		// Camera Index
	unsigned int  event_total;		// Total event amount in this search session
	unsigned int  event_start;		// event index, 0,1,2...;
								// because avSendIOCtrl() send package up to 1024 bytes one time, you may want split search results to serveral package to send.
	unsigned int  event_count;		// how much events in this package
